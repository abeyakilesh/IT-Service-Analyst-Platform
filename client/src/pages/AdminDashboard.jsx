        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-5 shadow-sm">
                    ...
                    <p className="text-2xl font-bold text-slate-900 dark:text-white">{summary?.avgResolutionTimeHours || 0}h</p>
                </div>
                <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-5 shadow-sm">
                    <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">SLA Compliance</p>
                    <p className="text-2xl font-bold text-emerald-600">{summary?.slaComplianceRate || 100}%</p>
                </div>
                <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-5 shadow-sm">
                    <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">In Progress</p>
                    <p className="text-2xl font-bold text-blue-600">{summary?.inProgressTickets || 0}</p>
                </div>
            </div>
            ...
            {summary?.ticketsByPriority && (
                <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-5 shadow-sm">
                    <h3 className="text-sm font-semibold text-slate-900 dark:text-white mb-4">Tickets by Priority</h3>
                    <div className="flex gap-6">
                        {Object.entries(summary.ticketsByPriority).map(([priority, count]) => (
                            <div key={priority} className="flex items-center gap-2">
                                <div className={`w-3 h-3 rounded-full ${priority === 'high' ? 'bg-red-500' : priority === 'medium' ? 'bg-amber-500' : 'bg-emerald-500'}`}></div>
                                <span className="text-sm text-slate-500 dark:text-slate-400 capitalize">{priority}</span>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                {tickets?.map((ticket) => (
                                    <tr key={ticket._id} className="hover:bg-blue-50/50 dark:hover:bg-slate-700/50 transition-colors">
                                        <td className="px-5 py-3 text-sm text-slate-700 dark:text-slate-300">{ticket.title}</td>
